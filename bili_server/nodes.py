...
class GraphNodes:
    def __init__(self,llm,retriever,retriever_grader,hallucination_chain,code_evaluator_chain,question_re_chain):
        self.llm = llm
        self.retriever = retriever
        self.retriever_grader = retriever_grader
        self.hallucination_chain = hallucination_chain
        self.code_evaluator_chain = code_evaluator_chain
        self.question_re_chain = question_re_chain
    ...
    def grade_documents(self,state):
        """
        重新表述输入问题以提高其清晰度和问题相关性，并使用转换后的问题更新图状态。
        ...
        for d in documents:
            score = self.retriever_grader.invoke({"input":question,"document": d.page_content})
            grade = score["score"]
            if grade == "yes":
    ...
    def transform_query(self,state):
        ...
        better_question = self.question_re_chain.invoke({"input":question})
